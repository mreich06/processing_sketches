...
void draw() {
  ...
  for (int j=0; j < particles.size(); j++) {
      Explosion curr = particles.get(j);
      curr.animate();
  }
}
//appends 100 circles to list
void draw_hundred() {
...
//appends explosions to list
void draw_expl(float isX, float isY) {
  for (int i=0; i<_explosion; i++) { //draw 10 circles
    Explosion myexpl = new Explosion(isX, isY);
    //mycircle.drawCircle();
    particles.add(myexpl);
    //explosion = (Explosion[])append(explosion, myexpl); //append circle just made to list
  }
}